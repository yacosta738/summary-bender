...
// Watch for changes in the theme selection and update the color mode accordingly
watch(() => colorMode.value, (newValue) => {
  document?.body?.classList.remove('light', 'dark') // Remove existing theme classes
  document?.body?.classList.add(newValue) // Add the new theme class
}, { immediate: true })
...
const { handleSubmit } = useForm({
  ...
  initialValues: {
    theme: colorMode.value === 'dark' ? 'light' : 'dark',
    font: 'inter',
  },
})

const onSubmit = handleSubmit((values) => {
  colorMode.value = values.theme
  toast({
